...
async def game_operation_lock(game_id: int):
    lock = None
    acquired = False
    #lock是一个Redis分布式锁对象,它的名字是"lock:game:{game_id}",超时时间是8秒(可以防止死锁),获取锁的最大等待时间是3秒.
    #acquired是一个布尔值,表示是否成功获取到了锁.
    try:
        ...
        lock = None
        acquired = False
    #如果成功获取到了锁就使用它,否则就退化为内存锁(不真正加锁,直接执行).无论如何都要yield让出控制权给调用者,等调用者执行完后如果是Redis锁还要释放锁.
...
async def pass_peng(game_id: int, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    async with game_service.lock_for(game_id):
        event_data = game_service.pass_peng(game_id=game_id, user_id=user.id)
        await save_game_to_cache(game_id)
    await persist_if_match_end(game_id, db)
    await broadcast_game_events(event_data)
    schedule_auto_progress(game_id)
